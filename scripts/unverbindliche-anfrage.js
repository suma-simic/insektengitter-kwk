...
(function () {
  ...
  if (hashRaw) {
    // Fall 1: #REF-05036 (ggf. gefolgt von weiterem Kram wie &zma_cid=..., z.B. durch
    // Zoho-Marketing-Automation-Link-Tracking angehaengt -- daher nur den REF-Teil matchen)
    if (/^REF-\d+/i.test(hashRaw)) {
      refFromHash = hashRaw.match(/^REF-\d+/i)[0];
    } else {
